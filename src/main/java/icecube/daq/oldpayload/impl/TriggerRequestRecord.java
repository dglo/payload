...
public class TriggerRequestRecord implements IWriteablePayloadRecord, Poolable {
    ...
    public void loadData(int iRecordOffset, ByteBuffer tBuffer) {
        ...
        // OFFSET_SOURCE_ID
        mt_sourceid = new SourceID(tBuffer.getInt(iRecordOffset + OFFSET_SOURCE_ID));

        //-read first time
        mt_firstTime = new UTCTime(tBuffer.getLong(iRecordOffset + OFFSET_FIRST_UTCTIME));

        //-read last time
        mt_lastTime = new UTCTime(tBuffer.getLong(iRecordOffset + OFFSET_LAST_UTCTIME));
