...
public class ReadoutRequestElementRecord implements IWriteablePayloadRecord, IReadoutRequestElement {
    ...
    public void loadData(int iRecordOffset, ByteBuffer tBuffer) {
        ...
        mt_sourceId = new SourceID(iSourceID);
        //-read first time
        mt_firstTime = new UTCTime( tBuffer.getLong(iRecordOffset + OFFSET_FIRST_TIME) );
        //-read last time
        mt_lastTime = new UTCTime(  tBuffer.getLong(iRecordOffset + OFFSET_LAST_TIME)  );
        //-read domid
        mt_domId = new DOMID( tBuffer.getLong(iRecordOffset + OFFSET_DOMID ) );
