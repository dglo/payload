    ...
    implements IWriteablePayloadRecord
{
    ...
    {
        firstTime = (IUTCTime) UTCTime8B.getFromPool();
        ((UTCTime8B)firstTime).initialize(timeVal);
    }

    public void setLastTime(long timeVal)
    {
        lastTime = (IUTCTime) UTCTime8B.getFromPool();
        ((UTCTime8B)lastTime).initialize(timeVal);
    }

    public void setSourceID(int srcVal)
    {
        srcId = (ISourceID) SourceID4B.getFromPool();
        ((SourceID4B)srcId).initialize(srcVal);
    }
